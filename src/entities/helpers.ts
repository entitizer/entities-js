...
export class UniqueNameHelper {
    ...
    static formatUniqueName(name: string): string {
        ...
        if (words.length === 1) {
            const fw = words[0];
            const lfw = fw[0].toLowerCase() + fw.substr(1);
            // Name->name === name
            if (lfw === fw.toLowerCase()) {
